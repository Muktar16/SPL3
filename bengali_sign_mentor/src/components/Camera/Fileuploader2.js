...
const FileUploader2 = () => {
  ...
  const handleSubmit = async () => {
    if (file) {
      ...
      try {
        const response = await axios.post("http://127.0.0.1:5000/word", formData);
        console.log(response);
        const foundObject = words.find(obj => obj.label === response?.data?.prediction);
  ...
  return (
    <div style={{display:'flex',flexDirection:'column'}}>
      ...
      {file && <Image style={{marginTop:'5px'}} src={URL.createObjectURL(file)} alt="Preview" width={200} />}
        <p style={{color:'green', fontSize:'35px',textAlign:'center'}}>{label1}</p>
        <Space>
